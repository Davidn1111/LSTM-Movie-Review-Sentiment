"""
Data preprocessing for LSTM training and testing
...
'''step 3: Get stats of length of sentences'''
...
def _convert_labels(input_data):
    ...
    value = {"pos": 1, "neg": 0}
    input_data['Label'] = [value[x] for x in input_data["Label"]]
...
def _map_tokens2index(input_data, top_K=500):
    ...
    # Define index for padding (0) and unknown (1)
    tokens2index['<pad>'] = 0
    tokens2index['<unk>'] = 1
...
def _pad_truncate_seq(x, seq_len):
    ...
    if len(x) >= seq_len:
        return x[:seq_len]
    # Add padding if sequence too short
    else:
        x.extend([0] * (seq_len - len(x)))
        return x
